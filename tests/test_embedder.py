...
class TestEmbedder(unittest.TestCase):
    ...
    def test_embedder(self):
        embeddings, metadata = self.embedder.get_embedding(self.samples)
        self.assertEqual(len(embeddings), len(self.samples))
        self.assertEqual(len(embeddings[0]), config.embedding_size)
        self.assertEqual(metadata['model'].replace(':', '-'), config.embedding_model)
